...
for ((i=0;i<${#PARSED_DATA[@]};++i)); do
    rm -r "${FILES[i]}/all_features/"* # Some files might linger on otherwise
    echo "Working on:  	${PARSED_DATA[i]} ${FILES[i]}"
    python ./selectCandidatesAndFeatures.py "${PARSED_DATA[i]}" "${FILES[i]}/all_networks/" "0" "$LENGTH_OF_TEST_USERS" "${FILES[i]}/all_features/"
done
